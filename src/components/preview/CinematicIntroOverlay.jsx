...
const CinematicIntroOverlay = () => {
  ...
  useEffect(() => {
    if (active) {
      ...
      const t2 = setTimeout(() => setStage(3), 300);  // Names
      const t3 = setTimeout(() => setStage(4), 800);  // Image fades in
      const t4 = setTimeout(() => setStage(5), 1900); // Fade out overlay
      const t5 = setTimeout(() => setStage(6), 2500); // Unmount
  ...
  const renderTextContent = (isOverlay = false) => (
    <div style={{ textAlign: 'center', padding: '0 20px', zIndex: 2, position: 'relative' }}>
      ...
      <div style={{
        ...
        <p style={{ fontFamily: 'var(--font-kr-sans)', fontSize: 'calc(0.9rem * var(--font-ratio))', letterSpacing: 'calc(0.1rem * var(--font-ratio))', marginTop: '6px', fontWeight: '300' }}>
          {mainInfo.location}
        </p>
      </div>
    </div>
  ...
  if (stage === 0 || stage === 6) return null;
  ...
  return (
    <>
      <div style={{
        ...
        opacity: stage >= 5 ? 0 : 1, 
        transition: 'opacity 0.7s ease-in-out, background-color 1s ease',
        pointerEvents: 'none',
        overflow: 'hidden'
      }}>
        ...
        <div style={{ position: 'relative', textAlign: 'center', zIndex: 10, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', width: '100%', height: '100%' }}>
          {/* Base Text Layer (Theme Text) */}
          <div style={{ color: theme.text }}>
            {renderTextContent(false)}
          </div>

          {/* Overlay Text Layer (White Text, Crossfade) */}
          <div style={{ 
            position: 'absolute', top: 0, left: 0, width: '100%', height: '100%',
            color: '#fff',
            opacity: stage >= 4 ? 1 : 0,
